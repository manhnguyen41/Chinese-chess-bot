def get_xiangqi_king_moves(current_pos, state, name):
    ...
    for dr, dc in directions:
        ...
        if name == 'k_1' and check_palace(new_row, new_col):
            if "1" not in state[new_row][new_col]:
                king_moves.append((new_row, new_col))

        if name == 'k_2' and check_palace(new_row, new_col):
...
def get_xiangqi_guard_moves(current_pos, state, name):
    ...
    for dr, dc in directions:
        ...
        # Check if the new position is within the 3x3 palace boundaries
        if name == 'g_1' and check_palace(new_row, new_col):
            if "1" not in state[new_row][new_col]:
                guard_moves.append((new_row, new_col))

        if name == 'g_2' and check_palace(new_row, new_col):
            if "2" not in state[new_row][new_col]:
                guard_moves.append((new_row, new_col))
...
def get_xiangqi_elephant_moves(current_pos, state, name):
    ...
    for dr, dc in directions:
        ...
        if name == "e_1" and check_board(new_row, new_col) and new_row <= 4:
            if state[row_near][col_near] == '':
                if "1" not in state[new_row][new_col]:
                    elephant_moves.append((new_row, new_col))

        # Check if the new position is within the board boundaries and not crossing the river
        if name == "e_2" and check_board(new_row, new_col) and new_row >= 5:
            if state[row_near][col_near] == '':
                if "2" not in state[new_row][new_col]:
                    elephant_moves.append((new_row, new_col))
...
def get_xiangqi_knight_moves(current_pos, state, name):
    ...
    for dr, dc in moves:
        ...
        if name == 'h_1' and check_board(new_row, new_col):
            if state[row_near][col_near] == '':
                if "1" not in state[new_row][new_col]:
                    knight_moves.append((new_row, new_col))

        if name == 'h_2' and check_board(new_row, new_col):
            if state[row_near][col_near] == '':
                if "2" not in state[new_row][new_col]:
                    knight_moves.append((new_row, new_col))
...
def get_xiangqi_pawn_moves(current_pos, state, name):
    ...
    direction = 1 if name == 'p_1' else -1
...
def get_xiangqi_rook_moves(current_pos, state, name):
    ...
    for i in range(1, 10):
        ...
        if state[new_row][col] == '':
            ...
            if '2' in state[new_row][col] and name == 'r_1':
                rook_moves.append((new_row, col))
                break
            if '1' in state[new_row][col] and name == 'r_2':
                rook_moves.append((new_row, col))
                break
            break
    ...
    for i in range(1, 10):
        ...
        if state[row][new_col] == '':
            ...
            if '2' in state[row][new_col] and name == 'r_1':
                rook_moves.append((row, new_col))
                break
            if '1' in state[row][new_col] and name == 'r_2':
                rook_moves.append((row, new_col))
                break
            break
...
def get_xiangqi_cannon_moves(current_pos, state, name):
    ...
    for i in range(1, 10):
        ...
        if len(cross_over) == 0:
            ...
            if state[new_row][col] == '':
                continue
            if '1' in state[new_row][col] and name == "c_2":
                cross_over.pop()
                cannon_moves.append((new_row, col))
                break
            if '2' in state[new_row][col] and name == "c_1":
                cross_over.pop()
                cannon_moves.append((new_row, col))
                break
    ...
    for i in range(1, 10):
        ...
        if len(cross_over) == 0:
            ...
            if state[row][new_col] == '':
                continue
            if '1' in state[row][new_col] and name == "c_2":
                cross_over.pop()
                cannon_moves.append((row, new_col))
                break
            if '2' in state[row][new_col] and name == "c_1":
                cross_over.pop()
                cannon_moves.append((row, new_col))
                break
